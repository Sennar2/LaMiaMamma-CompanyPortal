...
function weekStartMonday(ymd: string) {
  const [y, m, d] = ymd.split("-").map(Number);
  const dt = new Date(Date.UTC(y, (m ?? 1) - 1, d ?? 1));
  const wdStr = new Intl.DateTimeFormat("en-GB", {
  ...
  const parts = new Intl.DateTimeFormat("en-GB", {
    ...
    .formatToParts(start)
    .reduce(
      (o: any, p) => (p.type !== "literal" ? ((o[p.type] = p.value), o) : o),
      {}
    );
...
async function fetchDeptRevenueRows(depId: string, from: string, to: string) {
  ...
  while (true) {
    ...
    for (const r of arr) {
      const rawDate =
        r.date ?? r.businessDate ?? r.day ?? r.dateTime ?? r.dateUtc ?? "";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // BUDGETS / FORECASTS
    const budgetRows = await fetchBudgetsRows(
      departmentIds,
      weekStart,
      weekEndPlus1
    );
    ...
    let weekBudgetFull = 0; // Mon → Sun

    for (const r of budgetRows) {
      if (!r.date) continue;
      if (r.date === date) todayBudget += r.value;
      if (r.date >= weekStart && r.date <= yesterday)
        weekBudgetToDate += r.value;
      if (r.date >= weekStart && r.date <= weekEnd) weekBudgetFull += r.value;
    }

    const value = {
      todayActual,
      weekActual, // Mon → yesterday
      todayBudget,
      weekBudget: weekBudgetToDate, // legacy name: budget to date
      weekBudgetToDate,
      weekBudgetFull, // full week forecast
      weekForecast: weekBudgetFull, // legacy name used by UI
      meta: { used: "planday" },
    ...
    return NextResponse.json(
      { error: "Revenue fetch failed", detail: msg },
      { status: 502 }
    );
  }
}
