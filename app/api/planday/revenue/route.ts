...
function weekStartMonday(ymd: string) {
  ...
  const parts = new Intl.DateTimeFormat("en-GB", {
    ...
    .formatToParts(start)
    .reduce((o: any, p) => (p.type !== "literal" ? ((o[p.type] = p.value), o) : o), {});
    ...
    return arr.map((r: any) => {
      ...
      const v =
        typeof r.budget === "number"
          ? r.budget
          : typeof r.turnoverBudget === "number"
          ? r.turnoverBudget
          : typeof r.value === "number"
          ? r.value
          : typeof r.amount === "number"
...
export async function POST(req: Request) {
  ...
  try {
    const weekStart = weekStartMonday(date);          // Monday of this week
    const weekEnd = ymdPlusDays(weekStart, 6);        // Sunday of this week
    const weekEndPlus1 = ymdPlusDays(weekEnd, 1);     // Monday next week
    const tomorrow = ymdPlusDays(date, 1);            // for actuals fetch
    const yesterday = ymdPlusDays(date, -1);          // for "week so far"

    // ACTUALS: Mon → date (but "weekActual" only Mon → yesterday)
    let todayActual = 0;
    let weekActual = 0;
    for (const depId of departmentIds) {
      ...
      for (const r of rows) {
        ...
        if (r.date >= weekStart && r.date <= yesterday) weekActual += r.value;
      }
    }

    // BUDGETS / FORECASTS: full week budgets, plus "to date" if needed
    const budgetRows = await fetchBudgetsRows(departmentIds, weekStart, weekEndPlus1);
    let todayBudget = 0;
    let weekBudgetToDate = 0; // Mon → yesterday
    let weekBudgetFull = 0;   // Mon → Sun (full week forecast)

    for (const r of budgetRows) {
      if (!r.date) continue;
      if (r.date === date) todayBudget += r.value;
      if (r.date >= weekStart && r.date <= yesterday) weekBudgetToDate += r.value;
      if (r.date >= weekStart && r.date <= weekEnd) weekBudgetFull += r.value;
    }

    const value = {
      todayActual,
      weekActual,                 // Mon → yesterday (actuals)
      todayBudget,
      weekBudget: weekBudgetToDate, // keep old name = budgets to date
      weekBudgetToDate,
      weekBudgetFull,               // full week forecast
      weekForecast: weekBudgetFull, // legacy name used by UI = full week forecast
      meta: { used: "planday" },
