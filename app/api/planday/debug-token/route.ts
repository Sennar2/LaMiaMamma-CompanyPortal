// app/api/planday/debug-token/route.ts
...
export async function GET() {
  try {
    ...
    const scopes = payload.scope || payload.scp || payload.scopes || payload.Scopes || "";
    const exp = payload.exp ? new Date(payload.exp * 1000).toISOString() : null;
    return NextResponse.json({ scopes, exp, payloadExists: !!payloadB64 }, { status: 200 });
  } catch (e: any) {
