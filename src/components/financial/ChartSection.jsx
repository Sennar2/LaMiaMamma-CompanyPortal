...
export default function ChartSection({
  ...
  chartConfig,
  CSVLink,
  periodView, // "Week" | "Period" | "Quarter"
}) {
  ...
  const toPercentData = (rows, tab) => {
    return rows.map((row) => {
      ...
      if (tab === "Sales") {
        ...
        out.Sales_vsBudgetPct = budget > 0 ? ((actual - budget) / budget) * 100 : 0;
        ...
        out.Payroll_vsTheoPct = theo > 0 ? ((actual - theo) / theo) * 100 : 0;
      } else if (tab === "Food") {
        ...
        out.Drink_vsTheoPct = theo > 0 ? ((actual - theo) / theo) * 100 : 0;
      }
  ...
  const rawDisplayData =
    unit === "currency" ? filteredData : toPercentData(filteredData, activeTab);

  /**
   * ✅ KEY FIX:
   * In Period/Quarter mode your grouped rows only have Week = "P1"/"Q1" label.
   * So we compute an X label field that always exists.
   */
  const xAxisKey =
    periodView === "Period" ? "Period" : periodView === "Quarter" ? "Quarter" : "Week";

  const displayData = React.useMemo(() => {
    return (rawDisplayData || []).map((row) => {
      const fallbackLabel = row?.Week; // this is always present (and becomes P#/Q# in grouped mode)
      const preferredLabel = row?.[xAxisKey];
  ...
  }, [rawDisplayData, xAxisKey]);

  // Choose which lines to render in % mode
  let lines;
  if (unit === "percent" && activeTab === "Sales") {
    const salesActualLine =
      baseLines.find((l) => l.key === "Sales_Actual") || baseLines[0] || { color: "#4ade80" };

    const lastYearLine =
      baseLines.find((l) => l.key === "Sales_LastYear") || baseLines[2] || { color: "#f97316" };

    lines = [
      { key: "Sales_vsBudgetPct", color: salesActualLine.color, name: "Actual vs Budget" },
      { key: "Sales_vsLastYearPct", color: lastYearLine.color, name: "Actual vs Last Year" },
    ];
  } else if (unit === "percent" && activeTab === "Payroll") {
    const actualLine =
      baseLines.find((l) => l.key === "Payroll_Actual") || baseLines[0] || { color: "#4ade80" };

    lines = [{ key: "Payroll_vsTheoPct", color: actualLine.color, name: "Actual vs Theo" }];
  } else if (unit === "percent" && activeTab === "Food") {
    const actualLine =
      baseLines.find((l) => l.key === "Food_Actual") || baseLines[0] || { color: "#4ade80" };

    lines = [{ key: "Food_vsTheoPct", color: actualLine.color, name: "Actual vs Theo" }];
  } else if (unit === "percent" && activeTab === "Drink") {
    const actualLine =
      baseLines.find((l) => l.key === "Drink_Actual") || baseLines[0] || { color: "#4ade80" };

    lines = [{ key: "Drink_vsTheoPct", color: actualLine.color, name: "Actual vs Theo" }];
  } else {
  ...
  const tooltipFormatter = (value, name) => {
    const formatted = unit === "currency" ? formatCurrency(value) : formatPercent(value);
  ...
  return (
    <div
      style={{
        ...
        boxShadow: "0 24px 40px rgba(0,0,0,0.06), 0 4px 12px rgba(0,0,0,0.04)",
          ...
          >
            <button
              ...
              style={{
                ...
                boxShadow: unit === "currency" ? "0 6px 12px rgba(0,0,0,0.25)" : "none",
            ...
            <button
              ...
              style={{
                ...
                boxShadow: unit === "percent" ? "0 6px 12px rgba(0,0,0,0.25)" : "none",
                transition: "all 0.15s ease",
              }}
            >
              %
      ...
      <div style={{ width: "100%", height: "300px" }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={displayData}>
            {/* ✅ ALWAYS plot using __xLabel (works for Week + Period + Quarter) */}
